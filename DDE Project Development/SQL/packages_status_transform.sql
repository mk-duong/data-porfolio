WITH iteration_date AS (
  ...
  FROM `team-management-386803.jira_clv_staging.iteration_date_daniel_test`
)
...
, n1st_join_table AS (
  SELECT 
    ...
    , a.actual_status
    , p.planning_status
  FROM `team-management-386803.jira_clv_test.pkg_sts_actual_tracking` a
  LEFT JOIN `team-management-386803.jira_clv_test.pkg_sts_projection_tracking` p
    ON a.index = p.index
...
, pkg_sts_compare AS (
  SELECT 
    ...
    , sprint_status
  , CASE
      WHEN actual_status_decode = planning_status_decode THEN 'On Schedule'
      WHEN actual_status_decode < planning_status_decode THEN 'Behind Schedule'
